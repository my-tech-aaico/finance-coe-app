...
export function EntityTable({ entities, filters }: Props) {
  ...
  const updateFilter = useCallback(
    (key: string, value: string) => {
      ...
      startTransition(() =>
        router.replace(`/admin/entities?${params.toString()}`)
      );
    },
    [filters, router]
  );
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 page-header">
        <div>
          ...
          <p className="text-sm text-surface-400 mt-0.5">
            Manage legal entities for claim scoping.
          </p>
        </div>
                ...
                {entities.map((e) => {
                  ...
                  return (
                    <tr key={e.id} className="table-row">
                      ...
                      <td className="px-5 py-4 font-medium text-surface-800">
                        {e.name}
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`badge justify-center ${STATUS_COLORS[e.status] ?? ""}`}
                      ...
                      <td className="px-5 py-4 text-surface-500">
                        {e.updatedByName ? (
                          <span>
                            {e.updatedByName}
                            <span className="text-surface-400 text-xs block">
      ...
      {/* Modals */}
      <AddEntityModal open={addOpen} onClose={() => setAddOpen(false)} />
      {editEntity && (
        <EditEntityModal entity={editEntity} onClose={() => setEditEntity(null)} />
      )}
    </>
  );
}
