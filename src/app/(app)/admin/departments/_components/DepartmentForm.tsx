...
export function DepartmentForm({ editDepartment }: Props) {
  ...
  function handleSubmitClick(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    if (newCode === editDepartment!.code) return;

    e.preventDefault();
    const ok = window.confirm(
      `Rename "${editDepartment!.code}" to "${newCode}"? This code is displayed everywhere receipts use it — existing receipts will show the new code immediately. Continue?`
    );
    if (ok) {
      setConfirmed(true);
      setTimeout(() => {
  ...
  return (
    <div style={{ maxWidth: 480 }}>
      ...
      <p className="text-sm text-surface-400 mb-6">
        {isEdit ? "Update the department code or name." : "Create a new department for receipt categorization."}
      </p>

      <form ref={formRef} action={formAction} autoComplete="off">
        {isEdit && <input type="hidden" name="departmentId" value={editDepartment!.id} />}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            ...
            <p className="text-xs text-surface-400 mt-1">
              Lowercase, hyphen-separated. Used in receipt records and dropdowns. Convention: short keyword (e.g. <code>eng</code>, <code>mkt-asia</code>).
            </p>
          </div>
        ...
        <div className="flex gap-3 items-center">
          ...
          <a href="/admin/departments" className="btn-secondary">Cancel</a>
        </div>
      </form>
    </div>
  );
