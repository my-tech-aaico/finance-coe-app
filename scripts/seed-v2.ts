...
const PROJECT_CODES: { code: string; name: string }[] = [
  ...
  { code: "PRJ-311", name: "Data Warehouse Migration" },
...
async function seed() {
  ...
  for (const pc of PROJECT_CODES) {
    ...
    await db.insert(projectCode).values({ code: pc.code, name: pc.name });
    console.log(`Seeded project code: ${pc.code} — ${pc.name}`);
  }
